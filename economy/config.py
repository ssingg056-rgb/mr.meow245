...
class EconomyConfig:
    ...
    async def get_currency_info(self, guild_id: int) -> dict:
        """Return a dict with currency name and symbol.
        This matches the usage in the economy cog, which accesses
        the ``symbol`` key via ``["symbol"]``.
        """
        config = await self.get_config(guild_id)
        return {
            "name": config.get("currency_name", "Meow Coins"),
